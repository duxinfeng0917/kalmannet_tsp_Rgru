...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S, args):
        ...
        self.prior_S = prior_S.to(self.device) # 计算卡尔曼增益的中间变量 S = H*P_prior*H^T + R
        ...
        # GRU to track S
        self.d_input_S = self.n ** 2 + self.n ** 2 + 2 * self.n * args.in_mult_KNet
        ...
        # Fully connected 8
        self.d_input_FC8 =  self.n
        self.d_output_FC8 = 2 * self.n * args.in_mult_KNet
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        # FC 8
        # in_FC8 = torch.cat((obs_diff, obs_innov_diff), 2)
        ...
        # S-GRU
        in_S = torch.cat((out_FC1, out_R, out_FC7), 2)
    ...
    def init_hidden_KNet(self):
        ...
        # Initialize h_R with zeros since there's no prior_R available
        self.h_R = torch.zeros(self.seq_len_input, self.batch_size, self.d_hidden_R).to(self.device)
